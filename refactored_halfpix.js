...
Part.prototype.frame = function frame() {
  ...
  if (mouseDown) {
    ...
    if (clickDistance < radius) {
      ...
      this.x -= Math.floor(5 * dx / clickDistance);
      this.y -= Math.floor(5 * dy / clickDistance);
    }
  }
};
...
Part.prototype.displacementStyle = function displacementStyle(){
  ...
  alpha = alpha > 0.8 ? 0.8 : alpha; // maximum alpha
  alpha = alpha < 0.5 ? 0.5 : alpha; // minimum alpha
